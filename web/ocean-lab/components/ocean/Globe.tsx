...
export default function Globe({regions,region,flight,onArrive,onChoose}){
 ...
 return <div className="world-workspace"><div className="world-map"><div className="world-caption"><p className="kicker">GLOBAL MARITIME CONTEXT</p><h1>세계에서 해협까지.</h1><p>같은 바다 위, 서로 다른 연결 지점.</p></div>
 <svg className="world-globe" viewBox={small?"180 0 600 680":"0 0 960 680"} role="img" aria-label="드래그로 회전하는 세계 지구본" tabIndex={0}
 ...
 {continentLabels.filter(([,lon,lat])=>visible([lon,lat])).map(([name,lon,lat])=>{const p=projection([lon,lat]);return <text key={name} x={p[0].toFixed(3)} y={p[1].toFixed(3)} className="continent-label" textAnchor="middle">{name}</text>})}
 {Object.entries(regions).filter(([,r])=>visible(r.focus)).map(([key,r],i)=>{const p=projection(r.focus);const active=region===key;return <g data-region={key} key={key} transform={`translate(${p[0].toFixed(3)},${p[1].toFixed(3)})`} className="globe-pin" onClick={()=>!flight&&onChoose(key)}><title>{r.title+' 지도 열기'}</title><circle r="17" fill="transparent"/><circle r={active?10:6} fill="#c6f36c" fillOpacity={active?.24:1} stroke="#dffcb0"/><circle r="3" fill="#e9ffd0"/><text x={key==='hormuz'||key==='ulsan'?14:-14} y={key==='bab'?23:-13} textAnchor={key==='hormuz'||key==='ulsan'?'start':'end'}>{r.title}</text></g>})}
 </svg>
 {!land&&<p className="world-loading" role={error?'alert':'status'}>{error?'세계 지도를 불러오지 못했습니다. 지역 버튼으로 상세 지도를 열 수 있습니다.':'세계 해안선을 불러오는 중…'}</p>}
 <div className="world-areas" role="group" aria-label="대륙 시점">{areas.map(([id,label,center,zoom])=><Button key={id} variant="ghost" aria-pressed={area===id} disabled={!!flight} onClick={()=>{setArea(id);animate(center,zoom)}}>{label}</Button>)}</div>
 <div className="world-controls"><Button size="icon" variant="outline" aria-label="지구본 왼쪽 회전" disabled={!!flight} onClick={()=>move(-20,0)}><ArrowLeft/></Button><span>드래그 또는 방향키로 회전</span><Button size="icon" variant="outline" aria-label="지구본 오른쪽 회전" disabled={!!flight} onClick={()=>move(20,0)}><ArrowRight/></Button></div>
 <p className="world-source">Natural Earth 해안선 · 지구 곡면 투영 · 실제 고도·수심 미포함</p>
 {flight&&<div className="world-flight" role="status"><MapPin size={17}/>{regions[flight.region].title} 주변으로 이동 중</div>}
 </div><aside className="world-panel"><Globe2 size={27}/><p className="kicker">EXPLORE THE CONNECTIONS</p><h2>어디의 자료를<br/>살펴본 연구일까?</h2><p>위의 지역 버튼이나 지구본의 위치 표시를 선택하면 주변 지형과 자료 설명으로 이어집니다.</p><div className="world-evidence"><strong>6개 해외 통항 지역</strong><p>ONS의 통항량 자료가 다루는 지역입니다. 실제 항차별 급유 기록은 아닙니다.</p><strong>1개 국내 업무 자료</strong><p>울산항 벙커링정박지 신청현황으로 현장 업무변수를 살펴봤습니다.</p></div><p className="world-small">세계·지역 지도는 연구의 지리적 맥락을 설명합니다. 급유 실험은 별도의 합성환경 기록입니다.</p></aside></div>
}
