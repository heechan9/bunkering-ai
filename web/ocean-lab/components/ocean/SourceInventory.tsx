...
const sources=[
 ...
 {id:'kmou',ko:['해기원 실측 항차 자료','수령 대기','같은 항차의 출발잔량·실제급유량·소비량·도착잔량과 단위·식별자 필요'],en:['KMOU real-voyage records','Awaiting receipt','Need opening inventory, actual supply, consumption, closing inventory, units and shared identifiers'],path:'docs/technical/voyage_intake.md'},
];
export default function SourceInventory(){
 ...
 return <details className="voyage-review"><summary>{en?'Sources and confirmation status':'자료 출처·확인 상태'}</summary>
  <p>{en?'Project record as of 21 September 2026. Each link opens the supporting record at a fixed revision. Locally uploaded files are reviewed separately below.':'2026년 9월 21일 프로젝트 기록 기준입니다. 링크에서 해당 판단의 근거 기록을 확인할 수 있습니다. 직접 올린 파일은 아래 항차 CSV 검토에서 별도로 확인합니다.'}</p>
  <div className="review-table"><table><thead><tr>{(en?['Source','Confirmed status','Limits / needed information','Evidence']:['자료','확인 상태','한계·추가 필요 정보','근거']).map(h=><th scope="col" key={h}>{h}</th>)}</tr></thead><tbody>{sources.map(s=>{const v=en?s.en:s.ko;return <tr key={s.id}><th scope="row">{v[0]}</th><td>{v[1]}</td><td>{v[2]}</td><td><a href={root+s.path} target="_blank" rel="noreferrer">{en?'View record':'기록 보기'}</a></td></tr>})}</tbody></table></div>
 </details>;
}
