...
export default function OceanScene({progress,fuel,bunkering,view,resetCamera,onExportReady}){ const {t,lang,setLang}=useLocale();
 ...
 useEffect(()=>{
  ...
  function cylinder(g,r,h,x,y,z,m){const o=new THREE.Mesh(new THREE.CylinderGeometry(r,r,h,software?5:16),m);o.position.set(x,y,z);o.castShadow=true;g.add(o);return o}
  function line(g,points,color,width=1){const geom=new THREE.BufferGeometry().setFromPoints(points.map(p=>new THREE.Vector3(...p)));const o=new THREE.Line(geom,new THREE.LineBasicMaterial({color,linewidth:width}));g.add(o);return o}
  ...
  const safeLine=line(tankGroup,[[-4,2.0,-2],[5.8,2.0,-2],[5.8,2.0,2],[-4,2.0,2],[-4,2.0,-2]],'#ffbd66');
  ...
  function harbor(x,z,rotation){const g=new THREE.Group();g.position.set(x,-.1,z);g.rotation.y=rotation;scene.add(g);
