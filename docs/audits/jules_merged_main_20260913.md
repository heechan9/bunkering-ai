...
- **작성·정정 이력**:
  - 원본 감사 실행 및 보고서 작성: Jules (커밋 `9dc6946`)
  - 정정 검토: Codex ([PR #48 댓글](https://github.com/heechan9/bunkering-ai/pull/48#issuecomment-5651362094))
  - 정정 승인: 최희찬 (소유자)
  - 정정 반영: 현수(HyeonSuuuuu), Claude Code 보조. Jules 후속 커밋 `a6836d5`, `1e08663`은 변경 파일이 0개여서 정정이 반영되지 않았다.
- **정정 범위**: 본 정정은 보고서 서술만 수정하며 새 테스트·모델 실행이 아니다. 1절의 실행 결과(225 passed in 34.93s, 8/8, 종료코드 0)와 `.log` 원본은 그대로 유지한다.
...
*참고: 과거 보고된 225 passed는 이전 실행 이력이며, 상기 수치는 Jules가 본 실행 환경에서 직접 수행한 실행 로그의 결과입니다.*
...
제공된 실행 로그에서 전체 회귀 테스트 225개 및 논문 근거 계약 검사 8개가 통과했다. 본 검토에서 재현 가능한 확정 결함은 보고되지 않았다. 이는 모든 입력·실행 경로에 결함이 없음을 보증하지 않으며, 기존 테스트 통과만으로 모든 계약 준수를 입증할 수는 없다.
...
- **검증 한계**:
  - 원본 네 체크포인트와 전체 전이 묶음은 이번 GitHub 감사에 제공되지 않았다. 저장소에 커밋된 이전 Codex 감사 결과는 24개 파일 해시 대조와 40,120개 전이 재계산을 기록하고 있으나, 본 Jules 감사에서 이를 독립 재실행했다는 근거는 제출되지 않았다. 이 수치들은 기존 기록의 인용으로 구분하며, Jules 자체 검증 범위는 제출된 테스트 실행과 코드·문서 검토로 한정한다.
  - 기존 기록 출처(인용이며 이번 실행 로그가 아님): [`results/diagnostics/finalization_v3_0/snapshot_audit.json`](https://github.com/heechan9/bunkering-ai/blob/284b4487fed48d376fce16889e9faecb2f9253b1/results/diagnostics/finalization_v3_0/snapshot_audit.json)
...
- 중복 CSV 헤더의 명시적 검사는 `audit_diagnostic_snapshot.read_csv`에서 확인했다. `summarize_reward_diagnostics.load_run`의 중복 policy/seed/step 검사와 헤더 중복 검사를 구분한다. 모든 비정상 입력의 거부를 보장한다고 주장하지 않는다.
- 두 스크립트의 코드에는 NaN/Inf 수치, 필수 에피소드/step 누락, 비연속 step 번호, 체크포인트 해시 중복 검사가 있다. 본 감사에서 새로운 반례 입력 테스트를 실행하지는 않았다.
- `step_sci` 수식 재구성, `reward` 4성분 합산 정합성, 종료 사유(`end_reason` in `{"arrived", "fuel_depleted", "timeout"}`) 검증이 명확히 구현되어 있습니다.
- *구분*: 위 검사 항목이 위반 시 `ValueError`로 중단되도록 구현되어 있음을 코드 검토로 확인했다.

### B. 재고 수지 (Fuel Balance), SCI, 서명된 보상 성분 재구성
- 진단 코드는 기초 연료 + 총 실급유량 - 실현 소비량 - clipping 손실 - 기말 연료 = 0 수지를 기록하고 검사한다.
- 기존 기록 인용: [`results/diagnostics/finalization_v3_0/snapshot_audit.json`](https://github.com/heechan9/bunkering-ai/blob/284b4487fed48d376fce16889e9faecb2f9253b1/results/diagnostics/finalization_v3_0/snapshot_audit.json)은 40,120개 step 전이의 최대 step 수지 잔차를 $1.11 \times 10^{-16}$으로 기록하고 있다. 원본 네 체크포인트와 전체 전이 묶음은 이번 GitHub 감사에 제공되지 않았다. 저장소에 커밋된 이전 Codex 감사 결과는 24개 파일 해시 대조와 40,120개 전이 재계산을 기록하고 있으나, 본 Jules 감사에서 이를 독립 재실행했다는 근거는 제출되지 않았다. 이 수치들은 기존 기록의 인용으로 구분하며, Jules 자체 검증 범위는 제출된 테스트 실행과 코드·문서 검토로 한정한다.
...
- 감사 스크립트는 1,600개 원본 항차에서 반복된 규칙 기록 900개를 제거하고, 보존된 고유 규칙 조합 300개와 DQN 조합 400개를 합쳐 700개의 정책/체크포인트/시드 조합으로 집계한다.
...
- **후처리 허용오차 재분류**: $1 \times 10^{-12}$ 허용오차 적용 시 Safe Stock의 기존 기본조건 기록의 허용오차 재분류 미달률은 100%에서 0%가 된다. 이는 실선 안전성 검증이 아니다.
...
- 동일 기본조건에서 DQN의 SCI는 Safe Stock보다 53.97% 높고, 추가 구매량 0.46325는 추가 최종잔량과 같다. 그러나 SCI는 구매량뿐 아니라 구매 시점의 가격·환율에도 의존하므로 증가율 전체의 원인을 잔량 증가만으로 분해하거나 단정하지 않는다. SCI는 기말 재고 가치 차감이 적용되지 않은 합성 지표이다.
...
- 평가 모드(`evaluation/contract.py`, `scripts/evaluate.py`)는 DQN 모델 평가 시 `model.eval()` 호출 및 `torch.no_grad()` 환경에서 탐욕적(greedy) 행동을 선택한다. 평가 코드 및 해당 테스트 범위에서 탐욕적 추론과 가중치 불변을 검사한다. 원본 학습 체크포인트 네 개의 이번 독립 재실행은 미수행이다.
...
- 동일 선박/항로의 실제 급유 구매-소비-ROB(잔량) 실측 데이터 세트는 포함되어 있지 않으며, FuelCast, DataBio, 본 프로젝트의 설명용 웹 시연(Ocean Lab) 등은 공식 실험 입력 데이터가 아닙니다. 이 PR은 Ocean Lab 웹 소스·배포·브라우저 동작에 대한 감사가 아니다. 따라서 상용 적용 준비 완료(commercial readiness)를 주장할 수 없습니다.
...
1. **DQN 안전성 우위의 과장 위험**: Safe Stock의 안전 벌점(-0.5)은 십진수 $0.15$ 경계 오차에 기인한 것이므로, 기존 기본조건 기록의 허용오차 재분류 미달률 100%→0%를 DQN의 안전성 개선으로 해석하는 것은 타당하지 않다. 실선 안전성은 검증하지 않았다.
2. **SCI 및 비용 절감 오독 위험**: 동일 기본조건에서 DQN의 SCI는 Safe Stock보다 53.97% 높고, 추가 구매량 0.46325는 추가 최종잔량과 같다. 그러나 SCI는 구매량뿐 아니라 구매 시점의 가격·환율에도 의존하므로 증가율 전체의 원인을 잔량 증가만으로 분해하거나 단정하지 않는다. SCI 차이는 실제 화폐 비용 손실이나 절감을 입증하지 않는다.
...
1. **DQN 4개 체크포인트 바이너리 파일과 전체 전이 묶음**: 이번 감사에 제공되지 않았다. 체크포인트 재실행과 전이 재계산은 본 감사에서 수행하지 않았으며, 코드·문서 검토와 기존 기록 인용으로 한정했다.
...
**검토 범위 내 테스트 통과 및 확정 결함 보고 없음**.
Commit `284b448` 기준, 제공된 실행 로그에서 전체 회귀 테스트 225개 및 논문 근거 계약 검사 8개가 통과했다. 본 검토에서 재현 가능한 확정 결함은 보고되지 않았다. 이는 모든 입력·실행 경로에 결함이 없음을 보증하지 않으며, 기존 테스트 통과만으로 모든 계약 준수를 입증할 수는 없다.

### 결론 2: 접근 가능한 증거 기반 연구 주장 정당성 (Research Claims Justified by Accessible Evidence)
**조건부 정당 (JUSTIFIED WITH BOUNDARY LIMITATIONS)**.
- 제출된 테스트 실행과 코드·문서 검토 범위에서 보상 분해·연료수지 검사 로직을 확인했다. 40,120 step 전이의 연료수지 재계산은 저장소에 커밋된 이전 Codex 감사 기록(`snapshot_audit.json`)의 인용이며, 본 Jules 감사에서 독립 재실행한 결과가 아니다.
...
본 감사 PR은 기존 소스 코드, 환경 명세, 테스트 코드, 논문 텍스트, 기존 결과 파일을 변경하지 않으며 아래 2개 감사 전용 파일만 추가한다. 이후 정정 커밋은 1번 보고서 Markdown만 수정하며 2번 `.log`는 원본 그대로 보존한다.
