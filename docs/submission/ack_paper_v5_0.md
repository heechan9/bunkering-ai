# ACK 2026 병커시유 논문 v5.0

> 검토본: 저자 순서·영문 성명과 소속 번역·이메일은 최종 확정이 필요하다. 민재의 정성적 검토를 논의에 반영했으며 실험 수치와 모델은 변경하지 않았다.
