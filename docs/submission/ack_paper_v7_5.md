합성 항해환경에서 Double DQN 벙커유 구매정책의 민감도 및 연료수지 분석
...
Sensitivity and Fuel Balance Analysis of a Double DQN Bunker Purchasing Policy in a Synthetic Voyage Environment
...
본 연구는 합성 항해환경에서 규칙 기반 정책 3종과 Double DQN을 동일 조건으로 비교하고, 보상·구매량·잔량의 관계를 분석하였다. 네 학습 체크포인트를 동일한 100개 사례에 각각 적용해 소비량·항로 길이 민감도와 연료수지를 평가하였다. 소비량 +20%에서 DQN의 도착률은 100%였으나 SCI/step은 22.59% 증가하였다. 기본조건에서는 Safe Stock보다 SCI가 53.97% 높았고 추가 구매량은 추가 최종잔량과 일치했다. Safe Stock의 기존 안전선 미달 판정은 부동소수점 경계 효과로 확인하였다. 따라서 높은 보상과 도착률만으로 구매비용 우위를 판단할 수 없다. 한바다호 AB-LOG는 단위·집계 범위와 15개 가정 시나리오를 점검했으며 실측 정합성은 미확정이다. 본 결과는 합성환경의 평가 지표 해석에 한정하며 실제 비용절감이나 현장 성능을 입증하지 않는다.
...
본 연구는 합성 벙커링 의사결정 모델의 재현성과 소비량·항로 길이 민감도를 평가한다. 규칙 기반 정책과 Double DQN을 동일 조건에서 비교하고, 복수 학습 seed의 결과와 극단 사례를 분석한다. 연구 질문은 높은 보상·도착률이 낮은 구매비용 및 적정 재고와 일치하는가이다. 원본 행동 기록의 연료수지와 안전선 판정을 대조하여 비용·재고·보상의 불일치를 설명하고, 실선 자료를 정책 평가에 연결하기 위한 단위·시간 정합성 조건을 점검한다.
...
현재 모델은 항만별 공급 가능 여부·연료 규격, 급유·대기시간, 최소 주문량·고정 바지 수수료를 반영하지 않는다. 따라서 소량 반복 급유의 실무 실행 가능성이나 항만별 구매 권고의 타당성은 평가하지 못했다. step은 실제 일수로 보정되지 않았으며 급유 행동 횟수도 실제 기항 횟수가 아니다. 고정 안전선은 잔여 항해 소비량·사용 불가 잔량·연료 종류 차이를 충분히 표현하지 못한다. 실제 선속·기상·기관·추진효율·선종·탄소비용도 미반영이다. Suez/Cape는 horizon 확장 시나리오이며 실제 항로 재현이 아니다. 소비량 -10%의 초기 상한 손실 0.005 영향은 분리 검증하지 않았고, 안전선 재분류는 기본조건에 한정한다. AIS·기상·기관부하·소비량을 결합한 실측 검증자료는 확보하지 못했으며 실제 항차·에너지모델을 결합한 외부 검증이 필요하다[5].
...
본 연구는 합성환경에서 Double DQN과 규칙 기반 정책을 비교하고 소비량·항로 길이 민감도 및 연료수지를 분석하였다. DQN은 평가 범위에서 도착률 100%를 유지했으나 기본조건에서 Safe Stock보다 SCI와 최종잔량이 높았다. 추가 구매와 잔량의 관계 및 안전선의 수치 경계 효과를 확인하여 보상·구매량·잔량을 함께 평가할 필요성을 제시하였다[3]. 한바다호 기록은 정책 검증에 앞선 단위·시간 정합성 점검에 활용했다. 수정 환경 재학습과 현실 급유 제약을 포함한 외부 검증은 후속 과제이며 현재 결과로 실제 비용절감이나 현장 우위를 주장하지 않는다.
