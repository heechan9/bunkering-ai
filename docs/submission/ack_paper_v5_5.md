...
한국해양대학교 해기교육원에서 김승현을 통해 제공받은 한바다호 AB-LOG[6]를 별도로 검토하였다. AB-LOG 시트의 상단 ROB 670.3, 소비량 195.2, 하단 ROB 475.1 M/T는 급유가 없었다는 전달 내용을 조건으로 수지가 일치한다. 기관별 소비량 합계도 195.2 M/T이다. 다만 ROB 기준 시각과 소비량의 독립 측정 여부는 미확인으로, 내부 산술 일치를 실측 정확성 검증으로 해석하지 않는다.

B FOAM 시트 T8:V68의 기재 숫자 합계는 228.9이나 단위·기간·집계 범위가 확인되지 않아 요약표와의 대조를 보류하였다. 전달 메시지의 2025년과 파일·기록 날짜의 2026년도 확인이 필요하다. 빈 급유 칸을 0으로 대체하지 않았으며, 실제 구매량·가격 자료가 없어 급유정책이나 비용절감은 검증하지 않았다. 검토 기능은 원본을 변경하지 않고 셀 근거와 미확인 상태를 출력한다.
...
현재 모델은 실제 선속·기상·기관·추진효율·선종 및 탄소비용을 반영하지 않는다. Suez/Cape 조건은 horizon 확장으로 표현한 대표 시나리오이며 실제 항로·기상 재현은 아니다. 소비량 -10%에는 초기 상한 처리 손실 0.005가 포함되며 그 성능 영향은 분리 검증하지 않았다. 이번 안전선 재분류는 기본조건에 한정한다. SCI는 실제 통화비용이 아니며 실제 항차·에너지모델을 결합한 외부 검증이 필요하다[4]. 한바다호 기록의 내부 검산과 AIS·기상·기관부하를 결합한 외부 성능 검증을 구분한다.
...
본 연구는 동일조건 정책 비교, 복수 체크포인트 강건성 및 연료수지 진단을 제시하였다. DQN은 평가 범위에서 도착률 100%를 유지했으나 기본조건에서 Safe Stock보다 SCI와 최종잔량이 높았다. 안전선 경계 판정의 수치 효과를 분리함으로써 보상·SCI·잔량을 함께 해석할 필요성을 확인하였다. 한바다호 기록에서는 요약표의 조건부 수지 일치와 일별표 대조의 보류를 구분하였다. 이는 실제 비용절감의 증명이 아닌 후속 검증을 위한 평가 기반이다[3].
...
[6] 한국해양대학교 해기교육원 제공, 한바다호 AB-LOG, 파일 표기 2026년 5월, 미공개 자료, 2026.9.22 수령. 대상 연도 확인 중.
