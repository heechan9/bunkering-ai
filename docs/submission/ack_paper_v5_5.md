> 후속 집계 비교·가정 민감도 실험은 [v6.0 검토본](ack_paper_v6_0.md)에 반영했습니다. 아래는 이전 v5.5 정정본입니다.

# ACK 2026 병커시유 논문 v5.5 — 제공기관 회신 반영 정정본
...
본 연구는 동일조건 정책 비교, 복수 체크포인트 강건성 및 연료수지 진단을 제시하였다. DQN은 평가 범위에서 도착률 100%를 유지했으나 기본조건에서 Safe Stock보다 SCI와 최종잔량이 높았다. 안전선 경계 판정의 수치 효과를 분리함으로써 보상·SCI·잔량을 함께 해석할 필요성을 확인하였다. 한바다호 기록에서는 ROB 차감 소비량의 산술 관계를 확인했으며, 일별 kL와 요약 M/T의 환산·기간 정합성이 미확인되어 대조를 보류하였다. 이는 실제 비용절감의 증명이 아닌 후속 검증을 위한 평가 기반이다[3].
