...
학습 체크포인트 4개의 PC 진단과 원본 독립 대조를 완료했다. Windows 전체 테스트는
**215 passed in 37.78s**였다. 아래 과거 실행 기록의 “미완료”는 당시 상태다.
이후 같은 체크포인트의 Linux 재실행에서 Windows 원본 전이 CSV 4개가 바이트 단위로 일치했고, 정책·타깃·optimizer 상태 불변과 학습 호출 0회를 확인했다.
[재실행 및 제출 정리](submission_alignment_v3_0.md)에 고정 소스와 검증 근거를 연결한다.
현재 결과와 보완 자료의 기준은 [4-checkpoint 원본 대조](reward_diagnostics_results_4seed.md)다.

연료수지·보상·SCI가 코드 정의와 일치한다는 사실과 해당 정의가 실질 안전성을
올바르게 구분한다는 것은 별개다. 기본조건 Safe Stock의 안전선 위반 1 step은
0.1499999999999997과 0.15의 경계 오차였고 항차당 -0.5 보상 차이를 유발했다.
따라서 과거 H-2 및 아래 표를 실질적인 안전성 열세로 인용하지 않는다.
소량 반복 급유는 이제 실제 기록으로 관측됐으나 보상 설계의 인과효과는 미검증이다.

PR #44는 2026-09-13 병합 완료됐다(162c694d7c00bf4dc7edfdfa48486aaa56b6e356). 기존 재실행·회귀 미완료 항목은 해소됐고, 최신 변경의 검증 및 최종 통합 검토를 구분한다. 환경 판정 변경·재학습은 별도 변경 범위다.
...
이 표의 Safe Stock 미달은 원래 비교식의 경계 오차를 포함한다. 최신 판정을 함께 읽는다.
