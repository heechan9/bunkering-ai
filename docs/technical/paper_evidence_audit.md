...
- `source_commit_sha`: 감사 수행 시점의 checkout Git commit SHA (`git rev-parse HEAD`로 동적 추출되며, 감사 실행 시점의 현재 checkout 커밋 SHA를 나타냄; Git 미설치 또는 실행 불가 환경에서는 `"unavailable"`로 기록)
