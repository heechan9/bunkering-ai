...
- **주요 검증 항목**:
  ...
  7. **공식 Rule-based 평가 결과 (`CLAIM-007`)**: `results/evaluation_results.csv` 미존재 시 `missing_evidence`로 기록하며, 존재 시 `seed`, `episode`, `policy`, `reward`, `Synthetic Cost Index`, `success`, `fuel_depletion`, `bunkering_count`, `termination_reason` 필드 및 스키마 검증.
