...
- 기본 연료소비량은 기존과 동일한 정규화 값 `0.05/step`이다.
- `fuel_consumption_per_step` 생성자 인수는 `(0, 1]`의 유한값만 허용한다.
- 인수를 생략한 실행과 `0.05`를 명시한 실행은 동일 seed·action에서 같은
  전이 결과를 만들어야 한다.
- 이 값은 V1.5 강건성 평가를 위한 합성 전이 파라미터이며 실제 SFC, 톤,
  엔진부하, 선속 또는 추진효율이 아니다.
- 관측공간의 `sfc` 상수는 변경하거나 이 파라미터와 결합하지 않는다. 따라서
  소비량 변화는 정책이 직접 관측하지 못하는 외생 충격으로만 해석한다.
